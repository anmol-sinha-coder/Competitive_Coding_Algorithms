...
void Bubble_Sort()
{
 ...
 for(int i=1;i<=count+1 && tempi->next!=NULL;i++,tempi=tempi->next)
 {
  struct Node *tempj=head;
  for(int j=1;j<=count+1 && tempj->next!=NULL;j++,tempj=tempj->next)
...
int main()
{
 ...
 { // All positions start from 1 !
  ...
  switch(ch)
  {
   ...
   break;
   case 5://Bubble_Sort();
    printf("Enter 2 positions: ");scanf("%d%d",&p,&q);
    swap(p,q);
   break;
   case 6:exit(0);
   break;
